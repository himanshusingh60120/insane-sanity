...
/**
 * A wall-clock allowance for a stage of the run.
 *
 * The tool used to have no notion of time and would happily spend four minutes
 * probing links, which on Vercel means a 504 and no result at all. A partial
 * answer delivered in forty seconds beats a perfect one that never arrives.
 */
 ...
 * With a budget attached, workers stop picking up new items once it runs out and
 * the unfinished entries come back as undefined, which callers report as
 * "not checked" rather than silently treating as passing.
 */
export async function pool(items, limit, worker, budget = null) {
  const results = [];
  let cursor = 0;
