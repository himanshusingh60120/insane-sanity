...
export default function technicalRules(doc, ctx = {}) {
  ...
  // ── TECH-06 · double spaces inside a single run of copy ─────────────────────
  // Scanned per text segment, never across a tag boundary. Two paragraphs, one
  // ending in a trailing space or &nbsp; and the next starting after inter-tag
  // whitespace, flatten into a string that looks like it contains a double
  // space when the copy itself contains nothing of the kind. The subtitle is
  // checked from its raw form because headings are whitespace-normalised
  // during parsing, and it is where the recurring "Analysis,␣␣2026" defect lives.
  const haystacks = [
    ...(doc.textSegments || [doc.rawText]).map((value) => ['Body copy', value]),
  ...
  // ── TECH-15 · non-breaking space left in the copy ───────────────────────────
  // A trailing &nbsp; at the end of a paragraph is Word-paste residue. It is
  // invisible, harmless to a reader, and worth cleaning — but it is not a double
  // space, and conflating the two is what made TECH-06 misfire.
  const nbspSegments = (doc.textSegments || []).filter((s) => s.includes('\u00a0'));
  if (nbspSegments.length) {
    const trailing = nbspSegments.filter((s) => /\u00a0\s*$/.test(s)).length;
    issues.push({
      ruleId: 'TECH-15',
      category: 'Formatting',
      severity: 'info',
      items: [],
      title: `Non-breaking spaces in the copy (${nbspSegments.length} run${nbspSegments.length === 1 ? '' : 's'})`,
      where: 'Body copy',
      found: nbspSegments
        .slice(0, 3)
        .map((s) => reveal(context(s, s.match(/\u00a0/)[0], 30)))
        .join('\n'),
      expected: 'Ordinary spaces',
      fix:
        trailing > 0
          ? `${trailing} of them sit at the end of a paragraph, which is the signature of a paste from Word. Replace every non-breaking space with an ordinary one.`
          : 'Replace non-breaking spaces with ordinary spaces unless one is deliberately holding a number and its unit together.',
    });
  }
