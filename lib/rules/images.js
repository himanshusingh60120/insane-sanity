...
import { pool, norm, createBudget } from '../util.js';

const MAX_IMAGES = 20;
const IMAGE_BUDGET_MS = 12000;

export default async function imageRules(doc, ctx = {}) {
  const issues = [];
  const images = doc.images.filter((i) => !/\.svg($|\?)/i.test(i.src));

  // ── IMG-01 · every content image loads ──────────────────────────────────────
  const budget = createBudget(ctx.imageBudgetMs || IMAGE_BUDGET_MS);
  const probed = (
    await pool(images.slice(0, MAX_IMAGES), 8, async (img) => ({ ...img, ...(await probe(img.src)) }), budget)
  ).filter(Boolean);
