...
import { pool, unique, createBudget } from '../util.js';

const MAX_LINKS = 40;
const CONCURRENCY = 12;
const LINK_BUDGET_MS = 22000;
 ...
 */
export default async function linkRules(doc, ctx = {}) {
  const issues = [];
  ...
  const budget = createBudget(ctx.linkBudgetMs || LINK_BUDGET_MS);
  const raw = await pool(toCheck, CONCURRENCY, async (l) => ({ ...l, ...(await probe(l.href)) }), budget);
  const probed = raw.filter(Boolean);
  const unchecked = toCheck.length - probed.length;
  const skipped = links.length - toCheck.length;
  ...
  issues.push({
    ...
    found: [
      brokenInternal.length + brokenExternal.length === 0
        ? 'every link checked returned a success status'
        : `${brokenInternal.length} internal and ${brokenExternal.length} external not responding`,
      unchecked ? `${unchecked} not reached inside the time budget` : '',
      skipped ? `${skipped} beyond the ${MAX_LINKS}-link cap` : '',
    ].filter(Boolean).join(' · '),
    expected: '',
    fix: '',
  });
  ...
  if (unchecked > 0) {
    issues.push({
      ...
      severity: 'warn',
      items: ['M18', 'M19'],
      title: `${unchecked} link${unchecked === 1 ? '' : 's'} could not be checked in time`,
      where: 'Link sweep',
      found: `${probed.length} of ${toCheck.length} checked before the ${Math.round((ctx.linkBudgetMs || LINK_BUDGET_MS) / 1000)}s budget ran out`,
      expected: 'Every link checked',
      fix: 'Usually one slow external host holding up the queue. Re-run, or check the remaining links by hand — this run cannot vouch for them.',
    });
  }
