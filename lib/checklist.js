 ...
 * --------------------------------------
 * A check may only produce `severity: 'fail'` — the thing that writes "No" into
 * the sheet — when it maps to an item on the editorial checklist, or to the
 * formatting standard that was explicitly asked for.
 *
 * Anything inferred from reading the site is an observation, not a defect.
 * It is reported as 'warn' (visible, never flips a column) or 'info' (recorded
 * in the JSON only). Two examples of the mistake this prevents: internal links
 * that use two URL shapes both resolve, so "Internal links working" is a pass;
 * a non-breaking space left by a Word paste is untidy, not a spelling error.
 * Flagging either as a failure is a QA tool inventing work.
 *
